...
class MyFAISS(FAISS, VectorStore):
    ...
    def similarity_search_with_score_by_vector(
        ...
        for j, i in enumerate(indices[0]):
            ...
            if (not self.chunk_conent) or (
                "context_expand" in doc.metadata and not doc.metadata["context_expand"]
            ):
            ...
            for k in range(1, max(i, store_len - i)):
                break_flag = False
                if (
                    "context_expand_method" in doc.metadata
                    and doc.metadata["context_expand_method"] == "forward"
                ):
                    expand_range = [i + k]
                elif (
                    "context_expand_method" in doc.metadata
                    and doc.metadata["context_expand_method"] == "backward"
                ):
                    expand_range = [i - k]
                else:
                ...
                for l in expand_range:
                    if l not in id_set and 0 <= l < len(self.index_to_docstore_id):
                        ...
                        if (
                            docs_len + len(doc0.page_content) > self.chunk_size
    ...
    def delete_doc(self, source: str or List[str]):
        try:
            if isinstance(source, str):
                ids = [
                    k
                    for k, v in self.docstore._dict.items()
                ...
                vs_path = os.path.join(
                    os.path.split(os.path.split(source[0])[0])[0], "vector_store"
                )
            if len(ids) == 0:
